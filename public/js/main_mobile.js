  ...
  $(document).on("click", ".like", function(e) {
    ...
    $.ajax({
      ...
      success: function(data){
        // console.log(obj.closest('.postWrapper').find('.total-like').get(0));
        obj.parent().find('.actv').removeClass('actv');
        obj.addClass("actv");
        ...
        if($('.total-like').length){
          //if(likecount == 0){
            likecount = likecount + 1;
            obj.closest('.postWrapper').find('.total-like').text(String(data[0])+' likes');
            obj.closest('.postWrapper').find('.total-dislike').text(String(data[1])+' dislikes');
            dislikecount = 0;
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        data = $.map(data, function(el) { return el });
        if($('.total-dislike').length){
          //if(dislikecount == 0){
            dislikecount = dislikecount + 1;
            obj.closest('.postWrapper').find('.total-dislike').html(String(data[1])+' dislikes');
            obj.closest('.postWrapper').find('.total-like').html(String(data[0])+' likes');
            likecount = 0;
          //}
        }
      }
